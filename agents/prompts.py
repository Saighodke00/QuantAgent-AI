...
8. METRICS CHANGER: Track the parameters that yield the highest Total Return and Sharpe Ratio. You MUST initialize and track `best_win_rate = 0.0` and `best_max_drawdown = 0.0` inside your scanning loops alongside `best_return`, `best_sharpe_ratio`, and `best_equity_curve`. In your final JSON block, print the actual optimized metrics (`best_win_rate` and `best_max_drawdown`). NEVER hardcode `"win_rate": 0.0` or `"max_drawdown": 0.0` in the output JSON!
